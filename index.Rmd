---
title: 'Project 1: Potential Connection between Hate Crimes and Police Killing'
author: "SDS322E"
date: ''
output:
  html_document:
...
##### As a student of social science, I am interested in the social factors that are responsible or/and have the potential of causing both hate crimes and police killing. Hence, the research questions of this project include (1) whether there is something in common between factors impact the number of hate crimes and police killing, and (2) exploring the relationship between the frequency of killing executed by law enforcement officers and the incidence rate of hate crimes.
...
```{r}
police_killings %>% full_join(hate_crimes,by=c("state"="state_abbrev"), suffix=c(".abbrev",".full_name")) %>% filter(is.na(.))
...
```{r}
hcrimes_pkillings %>% group_by(state) %>% summarize(number_of_cases=n()) %>% arrange(-number_of_cases)
hcrimes_pkillings %>% group_by(state) %>% summarize(number_of_cases=n()) %>% arrange(number_of_cases)
hcrimes_pkillings %>% group_by(state) %>% summarize(number_of_cases=n()) %>% summarize(mean_cases=mean(number_of_cases))
```

The top five states had the GREATEST number of police killing cases in 2015 were CA, TX, FL, AZ, and OK. The top five states had the LOWEST number of police killing cases in 2015 were CT, DC, ME, ND, NH, RI, SD, VT, and WY. The mean value of the cases were 9.24. Maximum=74 and minimum=1.


```{r}
hcrimes_pkillings %>% mutate(share_nonwhite_tract=100-share_white) %>% group_by(state) %>% mutate(share_nonwhite_state=mean(share_nonwhite_tract)) %>% select(state,share_nonwhite_state,share_non_white) %>% distinct()
hcrimes_pkillings %>% mutate(share_nonwhite_tract=100-share_white) %>% group_by(state) %>% mutate(share_nonwhite_state=mean(share_nonwhite_tract)) -> hcrimes_pkillings1
```

Variable 'share_non_white' from the "hate_crimes" dataset indicates share of the population that is not white at state level in 2015. Variable 'share_white' from the "police_killings" dataset indicates share of the population that is non-Hispanic white at tract level. The above coding is to convert the 'share_white' variable to the 'share_nonwhite_state' variable that can be compared to the 'share_non_white' variable. Thus, the new dataset is "hcrimes_pkillings1."

```{r}
hcrimes_pkillings1 %>% group_by(raceethnicity) %>% summarize(n_cases_re=n()) %>% mutate(per_re=n_cases_re/nrow(hcrimes_pkillings1))
```
...
```{r}
hcrimes_pkillings1 %>% filter(college>mean(college)) %>% group_by(raceethnicity) %>% summarize(n_cases_re=n()) %>% mutate(per_re=n_cases_re/nrow(hcrimes_pkillings1)) %>% mutate(sum_per_re=sum(per_re))
```
